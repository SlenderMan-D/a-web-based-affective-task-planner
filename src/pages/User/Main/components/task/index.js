...
export default () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value, item) => {        
        if (item.status == 'done'){
          if(item.finshedTime===item.deadLine) {return    level[item.feel]    }else{
          return level[item.feel + (moment(item.finshedTime).isBefore(moment(item.deadLine)) ? 1 : -1)]  } 
        } else{
          return null 
        }
           //console.log (moment(item.deadLine))
        }
  ...
  return (
    <div>
      <Tabs
        ...
        tabBarExtraContent={
          activeTab === 'todo' ?
            (<Button type="primary" onClick={newTask} disabled={showModel}>
              New Task
            </Button>)
            : null}>
